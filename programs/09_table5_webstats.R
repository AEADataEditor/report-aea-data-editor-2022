...
# dbl  (8): id, revision, downloads, views, unique_downloads, unique_views, volume, version_volume
#
# dbl (2): sumsize, filecount
# The number of uploads was hand-counted: 4


utilizationReport.z <- read_csv(file.path(zenodobase,"zenodo_data_2022.csv"))
sumstats.z          <- read_csv(file.path(zenodobase,"zenodo_data_2022_summary.csv")) %>%
...
webstats <- utilizationReport.z %>%
  summarize(totalDownloads = sum(unique_downloads),
            ...
            volume         = sum(volume/1024/1024/1024),
            uploads        = 4) %>%
...
update_latexnums("zenodototalVolumeGB",round(webstats$volume,0))
update_latexnums("zenodototalVolumeTB",round(webstats$volume/1024,1))
